...
class Config:
  ...
  def __getitem__(self, item: str | tuple[str, str] | list[str, str]):
    """
    :param item:
      item can be:
      'item': returns item from current section
      ['section']['item'] returns item from selected section
    :return:
    """
    ...
    try:
      ...
      try:
        ...
        if _section in self._regVars and _item in self._regVars[_section]:
  ...
  def __setitem__(self, key: str | list[str, str] | tuple[str, str], value: str):
    if isinstance(key, tuple) or isinstance(key, list):
      ...
      _section = self._currentSection
      _key = key.lower()
    self._currentConfig[_section][_key] = value

  def __contains__(self, item: str | list[str, str] | tuple[str, str]):
    if isinstance(item, tuple) or isinstance(item, list):
      assert len(item) == 2
      _section = item[0]
      _item = item[1].lower()
    ...
    return (_section in self._currentConfig
            and _item in self._currentConfig[_section]
            or _section in self._defaultConfig
            and _item in self._defaultConfig[_section])
  ...
  def writeConfigBack(self, exclude_list: set[str]|list[str]|tuple[str]  = () ):
    """
  ...
  def register(self, data: tk.Variable, field: str, encrypt_class: Optional[Type] = None):
    """
    Connect tk variables with config values.
    :param data:  tk variable
    :param field: name of the config setting
    :param encrypt_class: class of the encryptor class, like Fernet. Must have an encrypt/decrypt/generate_key mehtod
    :return:
    """
    _field = field.lower()
